...
class Upload(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stoprequest.isSet():
            try:
                ...
                retVal = self.insertFile(os.path.basename(filePath), "%Y%m%d-%H%M%S", None,'image/jpeg', filePath)
                if retVal is None
                    logging.info("Upload failed")
                    self.mQueue.put(filePath)
                else:
                    logging.info("Upload success")
                    #Write uploaded file to metafile
                    f = open(metaFilePath, 'a')
                    f.write(filePath)
                    f.close()
            except Queue.Empty:
                continue
    ...
    def insertFile(self, title, description, parent_id, mime_type, filename):
        """Insert new file.
